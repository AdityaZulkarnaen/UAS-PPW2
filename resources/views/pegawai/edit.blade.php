        ...
        <div class="mx-auto max-w-screen-xl">
            <form action="{{ route('pegawai.update', ['id' => $data->id]) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $data->email) }}" class="bg-gray-50 border @error('email') border-red-500 @else border-gray-300 @enderror text-gray-900 text-sm rounded-lg block w-full p-2.5" required>
                    @error('email')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
