        ...
        <div class="mx-auto max-w-screen-xl">
            <form action="{{ route('pekerjaan.update', ['id' => $data->id]) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $data->nama) }}" class="bg-gray-50 border @error('nama') border-red-500 @else border-gray-300 @enderror text-gray-900 text-sm rounded-lg block w-full p-2.5" required>
                    @error('nama')
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="4" class="bg-gray-50 border @error('deskripsi') border-red-500 @else border-gray-300 @enderror text-gray-900 text-sm rounded-lg block w-full p-2.5" autocomplete="off" required>{{ old('deskripsi', $data->deskripsi) }}</textarea>
                    @error('deskripsi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
